...
#SBATCH --time=6:00:00
#SBATCH --job-name=DFOILemm
#SBATCH --error=DFOILemm
#SBATCH --output=DFOILemm
...
#if [ ! -f ~/work/DFOIL_out/him_emm_DFOIL/$(echo "${LISTa[ID]}")\_$(echo "${LISTb[ID]}")\_$(echo "${LISTc[ID]}")\_$(echo "${LISTd[ID]}")\_hermathena_13_DFOIL ]; 
#then
...
grep --no-group-separator -A 1 -e $(echo "${LISTa[ID]}") -e $(echo "${LISTb[ID]}") -e $(echo "${LISTc[ID]}") -e $(echo "${LISTd[ID]}") -e 'hermathena_13' $f > ~/work/DFOIL_out/him_emm/$fbname\_$(echo "${LISTa[ID]}")\_$(echo "${LISTb[ID]}")\_$(echo "${LISTc[ID]}")\_$(echo "${LISTd[ID]}")\_hermathena_13.fa
...
FILES=""
for f in ~/work/DFOIL_out/him_emm/*$(echo "${LISTa[ID]}")\_$(echo "${LISTb[ID]}")\_$(echo "${LISTc[ID]}")\_$(echo "${LISTd[ID]}")\_hermathena_13*; do FILES="$FILES $f"; done

python ~/work/Programs/dfoil/fasta2dfoil.py $FILES -o ~/work/DFOIL_out/him_emm/$(echo "${LISTa[ID]}")\_$(echo "${LISTb[ID]}")\_$(echo "${LISTc[ID]}")\_$(echo "${LISTd[ID]}")\_hermathena_13_count -n $(echo "${LIST[ID]}")

echo calculating DFOIL
python ~/work/Programs/dfoil/dfoil.py --infile ~/work/DFOIL_out/him_emm/$(echo "${LISTa[ID]}")\_$(echo "${LISTb[ID]}")\_$(echo "${LISTc[ID]}")\_$(echo "${LISTd[ID]}")\_hermathena_13_count --out ~/work/DFOIL_out/him_emm/$(echo "${LISTa[ID]}")\_$(echo "${LISTb[ID]}")\_$(echo "${LISTc[ID]}")\_$(echo "${LISTd[ID]}")\_hermathena_13_DFOIL

echo removing fasta files
for f in ~/work/DFOIL_out/him_emm/*$(echo "${LISTa[ID]}")\_$(echo "${LISTb[ID]}")\_$(echo "${LISTc[ID]}")\_$(echo "${LISTd[ID]}")\_hermathena_13.fa; do rm $f; done
